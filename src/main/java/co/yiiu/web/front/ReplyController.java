...
public class ReplyController extends BaseController {
    ...
    public String save(Integer topicId, String content, HttpServletResponse response) throws Exception {
        ...
        if (user.getScore() + siteConfig.getCreateReplyScore() < 0) {
            throw new Exception("你的积分不足，不能评论");
        }
        ...
        if (topicId != null) {
            ...
            if (topic != null) {
                ...
                String des = freemarkerUtil.format(
                        scoreEventConfig.getTemplate().get(ScoreEventEnum.REPLY_TOPIC.getName()), params);
                ...
                if (user.getId() != topic.getUser().getId()) {
                    notificationService.sendNotification(getUser(), topic.getUser(), NotificationEnum.REPLY.name(),
                            topic, content);
                }
                //给At用户发送通知
                ...
                for (String u : atUsers) {
                    ...
                    if (!u.equals(user.getUsername())) {
                        ...
                        if (_user != null) {
                            notificationService.sendNotification(user, _user, NotificationEnum.AT.name(), topic,
                                    content);
                        }
                    }
                }
    ...
    public Result up(@PathVariable Integer id) throws ApiException {
        ...
        if (_reply == null) {
            throw new ApiException("回复不存在");
        }
        if (user.getId() == _reply.getUser().getId()) {
            throw new ApiException("不能给自己的回复点赞");
        }
