...
public class TopicController extends BaseController {
    ...
    public String create() throws Exception {
        ...
        if (user.isBlock()) {
            throw new Exception("你的帐户已经被禁用了，不能进行此项操作");
        }

        if (user.getScore() + siteConfig.getCreateTopicScore() < 0) {
            throw new Exception("你的积分不足，不能发布话题");
        }
     ...
     */
    @PostMapping("/save")
    public String save(Integer nodeId, String title, String url, String content, HttpServletResponse response,
        ...
        if (StringUtils.isEmpty(title)) {
            ...
            String des = freemarkerUtil.format(
                    scoreEventConfig.getTemplate().get(ScoreEventEnum.CREATE_TOPIC.getName()), params);
